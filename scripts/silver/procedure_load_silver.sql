...
SELECT 
    ...
    -- ✅ Fix for invalid order dates
    CASE 
        WHEN TRY_CAST(CAST(sls_order_dt AS VARCHAR(8)) AS DATE) < '1900-01-01' 
             OR TRY_CAST(CAST(sls_order_dt AS VARCHAR(8)) AS DATE) > '2050-01-01'
        THEN NULL
        ELSE TRY_CAST(CAST(sls_order_dt AS VARCHAR(8)) AS DATE)
    END AS sls_order_dt,

    -- ✅ Fix for invalid ship dates
    CASE 
        WHEN TRY_CAST(CAST(sls_ship_dt AS VARCHAR(8)) AS DATE) < '1900-01-01' 
             OR TRY_CAST(CAST(sls_ship_dt AS VARCHAR(8)) AS DATE) > '2050-01-01'
        THEN NULL
        ELSE TRY_CAST(CAST(sls_ship_dt AS VARCHAR(8)) AS DATE)
    END AS sls_ship_dt,

    -- ✅ Fix for invalid due dates
    CASE 
        WHEN TRY_CAST(CAST(sls_due_date AS VARCHAR(8)) AS DATE) < '1900-01-01' 
             OR TRY_CAST(CAST(sls_due_date AS VARCHAR(8)) AS DATE) > '2050-01-01'
        THEN NULL
        ELSE TRY_CAST(CAST(sls_due_date AS VARCHAR(8)) AS DATE)
    END AS sls_due_date,
